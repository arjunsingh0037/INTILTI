...
if ($context->valid) {
    ...
    if ($userlog = $DB->get_record('local_ltiprovider_user', array('toolid' => $tool->id, 'userid' => $user->id))) {
        ...
        //BY-Arjun for adding consumer's username

        $arr = json_decode($sourceid, true);
        if(!isset($_REQUEST['lis_result_sourcedid'])){
            $arr['data']['userid'] = $_REQUEST['user_id'];
            $serviceurl = $_REQUEST['launch_presentation_return_url'];
        }
        $arr['data']['username'] = $_REQUEST['ext_user_username'];
        $sourceid = json_encode($arr);
